...
class HistoryPlusPlus{
  ...
  initHistory(options){
    ...
    if(this.list.length == 0) {
      this._navigate(this.getActualUrl(),{},options.initialData)
    }else{
      if(this.position < this.list.length-1) this.enableForwardButton()
      this.correctUrlWithRetard()
    }
    window.addEventListener('popstate', event => {
      if(!history.state || this._subNavigation){
        this._subNavigation = false
        ...
        let subpos = this.subpos()
        subpos++
        this.list[this.position].sublist[subpos] = {url:newurl}
        this.list[this.position].subpos = subpos
        this.correctUrlWithRetard()
      ...
      if(pos == 0){
        ...
        if(!this.preventBack && this.preventBackList.length == 0 && !this._preventEvent && this.subpos() == -1){
          //Launch back event
          this.position--
          if(this.position <= -1){
        ...
        }else if(this.subpos() >= 0 && !this._preventEvent){
          this.list[this.position].subpos--
          //Sublist used for internal position navigation with hash
          if(this.options.popOnSubBack) this.list[this.position].sublist.pop()
        ...
        if(goBackToOrigin && !this.preventExit){
          ...
          setTimeout(() => {
            if(this.position+1 < this.list.length && !this._forwardButtonEnabled || this.getActualData().sublist.length-1 <= this.subpos()){
        ...
        this.originalHistory.back()
        if(this.subpos() != -1 && this.getActualData().sublist.length-1 > this.subpos()){
          //SUB forward
          this.list[this.position].subpos++
          console.log("SUB FORWARD");
        }else{
          this.position++
        ...
        setTimeout(() => {
          ...
          if(this.position+1 >= this.list.length && this.getActualData().sublist.length-1 <= this.subpos()) {
  ...
  subpos(){
    let subpos = this.list[this.position].subpos
    if(subpos === undefined) subpos = -1
    if(this.list[this.position].sublist === undefined) this.list[this.position].sublist = []
    while(subpos != -1 && this.list[this.position].sublist.length <= subpos) this.list[this.position].sublist.push({url:this.getActualUrl()})
    return subpos
  }
  ...
  correctUrl(){
    ...
    if(this.subpos() >= 0){
      url = this.list[this.position].sublist[this.subpos()].url
    }
  ...
  getActualUrl(){
    return HistoryPlusPlus.getActualUrl()
  }
  ...
  getActualUrlNoHash(){
    return HistoryPlusPlus.getActualUrlNoHash()
  }
  ...
  getActualHash(){
    return HistoryPlusPlus.getActualHash()
  }
